...
export class State{
    ...
    webSocket$: Subject<any> = Backend.downloadFluxApps.connectWs()
...
export class View implements VirtualDOM{
    ...
    selected$ = new ReplaySubject<FluxApp>()
...
function treeItemView(state: ExplorerState, node: ModuleExplorer.Node) {
    ...
    if(node instanceof  ModuleExplorer.FileNode && node.file['metadata'] && node.file['metadata'].kind == 'flux-project' ){
        return {
            ...
            children:[
                ...
                {   
                    onclick: () =>  Backend.downloadFluxApps.publish$(node.id).subscribe(),
                    class: 'fas fa-cloud-download-alt fv-hover-text-focus mx-2 fv-pointer'
                },
                child$(
                    state.status$.pipe( 
